...
class CircularQueueTest {
    ...
    fun `zero capacity queue throws an exception`() {
        assertFailsWith(IllegalArgumentException::class) { CircularQueue<Unit>(capacity = 0) }
    }

    //region peek
    @Test
    fun `peek on an empty queue returns null`() {
        val queue = CircularQueue<Unit>(capacity = 1)
    ...
    fun `peek on a non-empty queue returns last added element`() {
        val queue = CircularQueue<Int>(capacity = 1)
        queue.add(69)
    ...
    fun `adding to already filled queue overwrites first value`() {
        val queue = CircularQueue<Int>(capacity = 3)
        queue.add(69)
        queue.add(112)
        queue.add(420)
        queue.add(69)
